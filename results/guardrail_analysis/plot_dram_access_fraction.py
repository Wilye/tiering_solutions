#!/usr/bin/env python3
"""Plot DRAM access fraction guardrail violations."""
import os
import re
import sys
import matplotlib
...
def plot_workload(results, results_dir, workload, workload_label):
    ...
    fig.suptitle(f'{workload_label} — DRAM Access Fraction Violations', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'dram_access_fraction_{workload}.png'), dpi=150)
    plt.close()
...
def plot_summary(results, results_dir):
    ...
    fig.suptitle('DRAM Access Fraction Violations (1:8 Ratio)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'dram_access_fraction_summary.png'), dpi=150)
    plt.close()
