#!/usr/bin/env python3
"""Plot migration effectiveness guardrail violations."""
import os
import re
import sys
import matplotlib
        ...
        for fname in filenames:
            ...
            for wl_key, _ in WORKLOADS:
                if fname.startswith(wl_key + "_"):
                    ...
                    if config == "normal":
                        continue  # skip normal, use cba_on as baseline
...
def plot_workload(results, results_dir, workload, workload_label):
    ...
    fig.suptitle(f'{workload_label} — Migration Effectiveness Violations', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'mig_eff_{workload}.png'), dpi=150)
    plt.close()
...
def plot_summary(results, results_dir):
    ...
    fig.suptitle('Migration Effectiveness Violations (1:8 Ratio)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'mig_eff_summary.png'), dpi=150)
    plt.close()
